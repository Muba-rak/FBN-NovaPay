...
function getChannelIcon(channel: Transaction['channel'], type: Transaction['type']) {
  switch (channel) {
    case 'pos_terminal':
...
export const TransactionRow = React.memo<TransactionRowProps>(({ transaction, onSelect }) => {
  ...
  return (
    <div
      ...
      className="group flex w-full items-center justify-between rounded-xl border border-transparent bg-white p-3.5 sm:p-4 text-left shadow-xs transition-all duration-150 hover:border-slate-300 hover:bg-slate-50/80 hover:shadow-sm focus-visible:border-amber-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400/40 dark:bg-[#112240] dark:hover:border-slate-700 dark:hover:bg-[#152a4e] cursor-pointer"
    >
      ...
      <div className="flex items-center space-x-3.5 min-w-0 pr-2">
        ...
        <div
          className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl transition-colors ${
            isFailed
              ? 'bg-red-50 text-red-600 dark:bg-red-950/50 dark:text-red-400 ring-1 ring-red-200 dark:ring-red-900/40'
              : isCredit
              ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-950/50 dark:text-emerald-400 ring-1 ring-emerald-200 dark:ring-emerald-900/40'
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-0.5 flex items-center space-x-2 text-xs text-slate-500 dark:text-slate-400">
            ...
            <span>•</span>
            <span className="shrink-0">{formatTransactionDate(transaction.createdAt)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center space-x-3 shrink-0 text-right">
        <div>
          ...
          >
            {isFailed ? formatKoboToNaira(transaction.amountKobo) : formattedAmount}
          </div>
